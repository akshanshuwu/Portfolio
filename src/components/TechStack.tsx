...
const TechStack = () => {
  ...
  const techSpheres = useMemo(() => {
    // Optimized: Reduce sphere count for better performance
    const items = [];
    // Use fewer spheres on mobile (15) vs desktop (20)
    const count = isMobile ? 15 : 20;
    
    // Pre-load textures to avoid duplicate loading
    const textureCache = new Map();
    
    for(let i=0; i<count; i++) {
        const tech = techList[i % techList.length];
        
        // Reuse textures instead of loading them multiple times
        let texture = textureCache.get(tech.name);
        if (!texture) {
          texture = textureLoader.load(tech.img);
          textureCache.set(tech.name, texture);
        }
